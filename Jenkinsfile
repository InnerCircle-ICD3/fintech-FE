pipeline {
    ...
    stages {
        ...
        stage('Validate Parameters') {
            steps {
                script {
                    env.DOCKER_REGISTRY = params.DOCKER_REGISTRY?.trim() ? params.DOCKER_REGISTRY : env.DEFAULT_REGISTRY
                }
            }
        }
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub-creds', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD')]) {
                    sh """
                        ...
                        docker push ${env.DOCKER_REGISTRY}/${IMAGE_NAME}:${BUILD_NUMBER} || echo "Docker 이미지 푸시 건너뜀"
                        docker push ${env.DOCKER_REGISTRY}/${IMAGE_NAME}:latest || echo "Docker 이미지 푸시 건너뜀"
                    """
                }
            }
        }
        ...
        stage('Deploy to Kubernetes') {
            steps {
                withCredentials([file(credentialsId: 'kubernetes-config', variable: 'KUBECONFIG')]) {
                    sh """
                        sed -i "s|\\\\\${DOCKER_REGISTRY}|${env.DOCKER_REGISTRY}|g" kubernetes/deployment.yaml || echo "배포 파일 수정 건너뜀"
                        sed -i "s|\\\\\${VERSION}|${BUILD_NUMBER}|g" kubernetes/deployment.yaml || echo "배포 파일 수정 건너뜀"
                        kubectl apply -f kubernetes/deployment.yaml -n ${env.K8S_NAMESPACE} || echo "Kubernetes 배포 건너뜀"
                        kubectl apply -f kubernetes/service.yaml -n ${env.K8S_NAMESPACE} || echo "Kubernetes 서비스 배포 건너뜀"
                        kubectl rollout status deployment/${IMAGE_NAME} -n ${env.K8S_NAMESPACE} || echo "배포 상태 확인 건너뜀"
                    """
                }
            }
        }
